...
from fluxvla.datasets.utils.video_decode import (
    clear_torchcodec_decoder_cache, decode_video_frames,
    get_torchcodec_decoder_cache_stats, reset_torchcodec_decoder_cache_stats)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--glob',
        default=
        'datasets/SARM_manual_test_10Episodes_lerobotv3.0/videos/*/chunk-000/*.mp4',
    ...
    *,
    ...
    if backend == 'torchcodec':
        if torchcodec_cache_size is None:
            raise ValueError(
                'torchcodec_cache_size must be set for torchcodec')
        os.environ['TORCHCODEC_DECODER_CACHE_SIZE'] = str(
            torchcodec_cache_size)
    ...
    if cache_stats is not None:
        ...
        hit_rate = (
            cache_stats['hits'] / cached_requests if cached_requests else 0.0)
