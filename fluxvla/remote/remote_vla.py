...
class RemoteVLAZmq:
    ...
    def __init__(
        ...
        self._socket = self._context.socket(
            zmq.REQ)  # 创建 REQ (Request) socket: 发一条请求,等一条响应
        self._socket.setsockopt(zmq.RCVTIMEO,
                                self._timeout_ms)  # 设置接收超时,防止 recv() 永久阻塞
        self._socket.setsockopt(zmq.SNDTIMEO,
                                self._timeout_ms)  # 设置发送超时,防止 send() 在网络不通时卡住
    ...
    def close(self):
        ...
        self._closed = True
        if hasattr(self,
                   '_socket') and not self._socket.closed:  # socket 存在且未关闭
