...
def parse_args():
    ...
    parser.add_argument(
        '--ckpt-path', required=True, help='Path to model checkpoint (.pt)')
...
def main():
    ...
    if dataset_key:
        ...
        if eval_cfg and hasattr(eval_cfg, 'denormalize_action'):
            ...
            print(f'[serve] Denormalize action transform built '
    ...
    from .vla_server import VLAInferPipeline, create_vla_server
    ...
    print(f'[serve] Model on {args.device} ({args.dtype}), ready to serve.')
